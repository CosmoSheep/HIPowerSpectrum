...
class b_sink:
	...
	def __init__(self,  z_re, z_obs, scenario=None):
		...
		self.fold_cr="/fs/scratch/PCON0003/osu10670/cr-1/"
		self.fold_cr_nov="/fs/scratch/PCON0003/osu10670/cr-nov-1/"
	...
	def xi_his(self, CRs, alphaBs):
		...
		xi_arr = interpolate.interp1d(np.arange(self.z_re, self.z_obs+2*self.dz,self.dz), np.array(xi_arr))
		xi_dots = interpolate.interp1d(np.arange(self.z_re, self.z_obs+2*self.dz,self.dz), np.array(xi_dots))
		CR_mean = interpolate.interp1d(np.arange(self.z_re, self.z_obs+2*self.dz,self.dz), np.array(CR_mean))
