...
#ifdef MULTI_IC
	  block[3 * pc + k] = P[i].GasPos[k];
#endif
#endif
	}
#ifdef USE_XBC
        block[3*pc] = periodic_wrap(block[3*pc]+USE_XBC);
#endif
...
#ifdef USE_VBC
        block[3*pc] += USE_VBC/sqrt(InitTime);
#ifdef COMPENSATE_V
        block[3*pc] -= USE_VBC/sqrt(InitTime) * OmegaBaryon/Omega;
#endif
#endif
